...
describe("feature contracts", () => {
  ...
  it("validates schema version, revision, resumable step and strict switches", () => {
    ...
    expect(FeatureConfiguration.safeParse({ ...input, walkthroughStep: 9 }).success).toBe(true);
    expect(FeatureConfiguration.safeParse({ ...input, walkthroughStep: 10 }).success).toBe(false);
    const update = { revision: 0, values, walkthroughComplete: false, walkthroughStep: 9 };
    expect(FeaturesUpdateRequest.safeParse(update).success).toBe(true);
    expect(FeaturesUpdateRequest.safeParse({ ...update, walkthroughStep: 10 }).success).toBe(false);
    expect(
