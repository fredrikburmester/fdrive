/**
 * A tiny ZIP encoder producing "stored" (uncompressed) entries, used by the
 * fake server to answer streamzip and public-share-zip requests without a
 * dependency. Implements just enough of the format: local file headers, a
...
/**
 * CRC-32 (ISO-3309 / zlib polynomial), computed bit by bit rather than via a
 * lookup table so there is no array indexing to reconcile with
...
/** Builds a ZIP archive (stored method, no compression) from the given entries. */
export function buildZip(entries: readonly ZipEntryInput[]): Uint8Array {
  ...
  for (const entry of entries) {
    ...
    const { dosTime, dosDate } = toDosDateTime(entry.mtime ?? new Date(0));
