...
it("publishes complete files under a real DAV lease with durable replay and conflict protection", async () => {
  ...
  try {
    ...
    const listing = (await json(`${base}/entries?path=/`)) as { entries: Array<{ path: string }> };
