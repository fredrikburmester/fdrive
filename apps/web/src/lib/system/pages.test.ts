...
import { AI_PAGE, FEATURE_PAGES, OFFICE_PAGE } from "./pages";
...
describe("system page links", () => {
  ...
  it("gives AI its own page outside the feature table", () => {
    expect(AI_PAGE).toEqual({ href: "/system/ai", label: "AI" });
    expect(Object.values(FEATURE_PAGES).map((page) => page.href)).not.toContain(AI_PAGE.href);
  });
});
